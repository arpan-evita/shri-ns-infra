...
export const PropertyCard = ({ property }: PropertyCardProps) => {
  return (
    <div className="group bg-[#0f0f0f] border border-white/5 rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-500 shadow-2xl">
      <Link to={`/properties/${property.slug}`} className="block relative h-72 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <div className="bg-primary text-black text-[9px] font-black px-3 py-1.5 uppercase tracking-[0.2em] rounded-md shadow-lg">
            {property.status === 'buy' ? 'For Sale' : 'For Rent'}
          </div>
          {property.possession_status && (
            <div className="bg-black/80 backdrop-blur-md text-white text-[9px] font-black px-3 py-1.5 uppercase tracking-[0.2em] rounded-md border border-white/10 shadow-lg">
              {property.possession_status}
            </div>
          )}
        </div>
        <div className="absolute bottom-4 left-4 right-4">
           <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-4 rounded-xl flex justify-between items-center transition-all duration-500 group-hover:bg-primary/20 group-hover:border-primary/30">
              <div className="text-white font-black text-xl">
                ₹{property.price ? property.price.toLocaleString('en-IN') : 'Price on Request'}
              </div>
              <div className="text-slate-300 text-[10px] font-bold uppercase tracking-widest bg-white/5 px-2 py-1 rounded">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-1 bg-white/[0.03] p-1 rounded-xl">
          <div className="bg-[#151515] p-3 text-center rounded-lg space-y-1">
